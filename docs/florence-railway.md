...
- Florence serves HTTP from `florence-server`
- Health check: `/health`
...
3. Set `DATABASE_URL` or `FLORENCE_DATABASE_URL` on the Florence service from that Postgres instance.
4. Expose the generated Railway domain or attach your own custom domain.
...
5. Confirm Florence can read and write the Railway Postgres database.

## Operational Notes

- Florence's background Google sync runs inside the web process.
- Postgres removes the main SQLite single-replica limitation, but for the demo
  I would still keep Florence at one replica until the Linq flow is stable.

...
- Pre-upgrade messages without urgency metadata stay conservatively deduplicated instead of being replayed; new messages distinguish urgency per action. Unapproved pre-key private-Google Calendar proposals are retired once during migration and recreated by the next authoritative review with an exact action key; conversational actions and committed history are preserved.

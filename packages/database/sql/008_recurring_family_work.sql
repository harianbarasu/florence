-- Scheduled family work reuses the existing family_task lifecycle. Its schedule envelope lives in
-- reminder_schedule, while last_run_at records the most recently confirmed terminal delivery.
ALTER TABLE proactive_work
  DROP CONSTRAINT proactive_work_family_task_shape_check,
  DROP CONSTRAINT proactive_work_non_reminder_fields_check;
...
ALTER TABLE proactive_work
  ...
  ADD CONSTRAINT proactive_work_non_reminder_fields_check CHECK (
    kind IN ('reminder','family_task') OR (reminder_schedule IS NULL AND last_run_at IS NULL)
  );

...
const privateStableFactDecisionSchema = privateFactContextSchema
  .extend({ sourceIds: z.array(opaqueId).min(1).max(10) })
  .strict();
...
currentPrivateFacts contains only memory already private to this parent. Independently of the findings, return up to six facts only for durable family logistics that will remain useful over time, such as a school office contact, recurring pickup pattern, or standing activity detail. Use the same stable lowercase semantic slot to replace an earlier version, and cite only current Gmail or Calendar sourceIds returned by a read tool. Do not retain deadlines, one-off events, health or financial information, credentials, secrets, private adult matters, guesses, or anything merely interesting. An omitted existing fact remains unchanged.
...
When googleConnection.kind is personal, currentPrivateFacts contains only memory already private to this parent. Independently of materialChange and findings, return up to six facts only for durable family logistics that will remain useful over time. Use the same stable lowercase semantic slot to replace an earlier version, and cite only sourceIds in the current bounded evidence. Do not retain deadlines, one-off events, health or financial information, credentials, secrets, private adult matters, guesses, or anything merely interesting. An omitted existing fact remains unchanged. When googleConnection.kind is family, facts must be empty and currentPrivateFacts will be empty.

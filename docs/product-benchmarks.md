...
_Research date: August 16, 2026. This report uses public first-party product pages, policies, documentation, and founder-authored material only. Authenticated Instinct UI observations are intentionally separated below._
...
> These are authorized observations from the user's signed-in Instinct workspace on August 16, 2026. They are not claims derived from public documentation.
...
- **Onboarding:** the already-authenticated workspace does not expose the original signup sequence, so no onboarding claim is made from this walkthrough.
...
- **Visual system:** the live UI uses `GT America, -apple-system, system-ui, sans-serif`. Measured light-theme tokens are white background/card, `#101113` primary text, `#e5e7eb` borders, `#f3f4f6` muted surfaces, primary text at 60% for secondary copy and 35% for tertiary copy, and a blue focus ring. The desktop shell has a 200px fixed sidebar, 48px header, and a centered 700px content column with 32px page padding and 636px inner content. Mobile removes the sidebar and retains a 48px header; Workspace cards use a 16px outer gutter while the Vault's narrow record list uses a 32px inner gutter.

...
- **Hermes Agent at the same commit, `tests/skills/test_google_workspace_api.py`: assertion and reference adaptation** for request/result shape and Gmail/Drive behavior. Its expectations informed provider-fake checks and Florence's focused durable retry and Calendar checkpoint regressions; the Python CLI harness itself was not imported.
...
Deliberate remaining limits are explicit rather than overclaimed: arbitrary Drive binary/PDF download and upload, Gmail attachments/drafts/forwarding, and group-created work routed through its initiating adult's personal Google account are not implemented here. Those belong to the authenticated browser/forms and communication/real-errand tickets.

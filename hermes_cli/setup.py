...
def _print_setup_summary(config: dict, hermes_home):
    ...
    # Web tools (Exa, Parallel, Firecrawl, Tavily, OpenAI, or Anthropic)
    try:
        from agent.anthropic_adapter import resolve_anthropic_token
        _anthropic_web_available = bool(resolve_anthropic_token())
    except Exception:
        _anthropic_web_available = bool(get_env_value("ANTHROPIC_API_KEY") or get_env_value("ANTHROPIC_TOKEN"))

    if subscription_features.web.managed_by_nous:
        tool_status.append(("Web Search & Extract (Nous subscription)", True, None))
    elif subscription_features.web.available or get_env_value("OPENAI_API_KEY") or _anthropic_web_available:
        label = "Web Search & Extract"
        provider_label = subscription_features.web.current_provider
        if not provider_label:
            if get_env_value("OPENAI_API_KEY"):
                provider_label = "OpenAI"
            elif _anthropic_web_available:
                provider_label = "Anthropic"
        if provider_label:
            label = f"Web Search & Extract ({provider_label})"
        tool_status.append((label, True, None))
    else:
        tool_status.append(("Web Search & Extract", False, "EXA_API_KEY, PARALLEL_API_KEY, FIRECRAWL_API_KEY/FIRECRAWL_API_URL, TAVILY_API_KEY, OPENAI_API_KEY, Anthropic credentials, SEARXNG_URL, BRAVE_SEARCH_API_KEY, or ddgs"))

...
Florence already has the right household-specific foundation: two-parent Messages participation, a parental-unit knowledge model, 90-day Gmail and Calendar ingestion, reminders, durable work with exact future rechecks, structured public tools, broad Google Workspace operations, authenticated browser work, and outbound phone/SMS providers. The largest remaining product gap is not agent infrastructure. It is repeatedly proving **complete cross-channel jobs through their final provider result or receipt**.
...
| Durable/proactive work | Checkpoints, exact future rechecks, quiet unchanged checks, retry, steering, cancellation, progress, reminders, finite monitors, and terminal delivery through the existing due-work path. ([Reasoner](../../apps/api/src/reasoner.ts), [store](../../packages/database/src/store.ts)) | Thinks/plans/acts, follows dropped threads, and proactively contacts the user. | Prove the new send/wait/recheck path against live replies and broaden it across provider processing, availability changes, and travel disruption—not another scheduler or task runtime. |
...
2. **Cross-channel continuation:** rehearse the implemented future-recheck path beginning with Gmail or Calendar, using browser/call/text as needed, and reconciling the provider result back into the same family task.

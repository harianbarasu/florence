...
The same task can also stop polling when useful work depends on outside state and resume at one exact future instant. A model-visible `deferred` result writes that instant into the existing `proactive_work.next_check_at`; the task remains active, steering wakes it immediately, cancellation removes the wake, and an unchanged later check stages no status message. The controlling text journey sends the dentist one message, confirms carrier delivery, checks for a reply at two persisted times without resending or repeating “still waiting,” and finishes from the returned message. Florence exposes the next check time in ordinary task status so a parent can ask when she will look again.
...
- Hermes Agent at the same commit, `cron/jobs.py` and `skills/productivity/product-price-monitor/SKILL.md` — adapted the applicable one-shot scheduling and unchanged-result suppression practice for future rechecks. Hermes's JSON cron store, ticker, and scheduler provider were not ported because Florence already persists, claims, fences, and delivers due work in PostgreSQL.

### Verification

- The 2026-08-28 future-recheck extension passes `pnpm check`: lint, every package/app typecheck and build, the Google package test, and 55 API tests with the five PostgreSQL-gated tests skipped because `TEST_DATABASE_URL` is not configured in this checkout.
- The focused reasoner journey proves one `sms_send`, one delivery check, two future `sms_inbox` reads, a quiet unchanged defer, and one transcript-backed terminal answer. The operation list asserts that the original text is never resent.
- The original durable-work tranche passed its then-current `pnpm check`: lint, all package/app typechecks, 31 default tests with database-gated journeys skipped, and all builds.

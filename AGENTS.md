...
- A cold-added group is observe-only from the moment Florence joins: new ordinary content is retained only in that exact participant epoch, Florence emits absolutely nothing into the group, and registered exact-chat participants receive only independent private source access. Group writing requires every current participant to be a registered Florence user plus an exact current-audience approval. Every participant change starts a new immutable epoch, invalidates write rules, and falls back to observe-only.

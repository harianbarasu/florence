    ...
    return inTransaction(this.database, async (transaction) => {
      ...
      const updated = await transaction<{ readonly id: string }[]>`
        update coverage_loops set
          ...
          time_zone = ${decision.loop.timing.timeZone},
          local_date = ${decision.loop.timing.localDate},
          event_at = ${asDate(decision.loop.timing.eventAt)},
          deadline_at = ${asDate(decision.loop.timing.deadlineAt)},
          preparation_minutes = ${decision.loop.timing.preparationMinutes},
          travel_minutes = ${decision.loop.timing.travelMinutes},
          earliest_useful_at = ${new Date(decision.loop.timing.earliestUsefulAt)},
          last_responsible_at = ${new Date(decision.loop.timing.lastResponsibleAt)},

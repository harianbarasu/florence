...
**Current Florence reality:** most of the durable-work plumbing exists, including conversational outbound calls, status, cancellation, persisted provider handles, transcript/summary results, and duplicate-call protection ([telephony adapter](../../apps/api/src/telephony.ts), [reasoner tools](../../apps/api/src/reasoner.ts)). A mocked dentist narrative reaches a transcript-backed appointment result ([communication narrative](../../apps/api/src/reasoner-tool-loops.communication.test.ts)). However, production currently has no configured telephony provider and there is no live-provider rehearsal. Before calling becomes a product claim, Florence needs one real end-to-end call plus a stable caller identity, mandatory “Florence calling for this parent” disclosure, explicit recording behavior and retention controls, action-appropriate approval, and inspectable/deletable call evidence. The first acceptance run should be: call a pediatric dentist using the child's constraints and both adults' availability, book only within authorized options, return transcript-backed confirmation, and update the Family Calendar.

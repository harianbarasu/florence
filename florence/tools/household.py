...
class FlorenceHouseholdToolContext:
    ...
    visibility_scope: FlorenceConversationScope | None = None
...
def set_household_tool_context(
    ...
    *,
    ...
    visibility_scope = resolve_conversation_scope(
        store,
    ...
    with _context_lock:
        _tool_contexts[task_id] = FlorenceHouseholdToolContext(
            ...
            channel_type=visibility_scope.channel_type,
            visibility_scope=visibility_scope,
            allowed_source_scopes=_allowed_source_scopes(visibility_scope),
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "pending_action_id": {
                "type": "string",
                "description": (
                    "The active pending_action_id from the current DM prompt. Required for newly surfaced "
                    "review prompts that include a pending_action_id."
                ),
            },
            "resolution": {
                "type": "string",
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "pending_action_id": {
                ...
                "description": (
                    "The active pending_action_id from the current DM prompt. Required for newly surfaced "
                    "reminder prompts that include a pending_action_id."
                ),
            },
            "action": {
                "type": "string",
...
SEARCH_GOOGLE_INBOX_SCHEMA = {
    ...
    "description": (
        ...
        "Always inspect mirror_freshness before making latest, complete, or absence claims."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
SEARCH_GOOGLE_CALENDAR_SCHEMA = {
    ...
    "description": (
        ...
        "In the family group, only shared-household calendar scope is allowed. Always inspect mirror_freshness before "
        "making latest, complete, or absence claims."
    ),
    "parameters": {
        "type": "object",
        "properties": {
...
def _handle_apply_candidate_review(args: dict, *, task_id: str | None = None, **_: Any) -> str:
    ...
    binding_error = _validate_pending_action_binding(
        context,
        expected_action_type="candidate_review",
        target_id=candidate_id,
        requested_pending_action_id=_normalize_optional_text(args.get("pending_action_id")),
    )
    if binding_error is not None:
        return json.dumps(binding_error)
    ...
    review_context = _build_candidate_review_context(
        context,
        candidate_id=candidate_id,
...
def _active_nudge_id(context: FlorenceHouseholdToolContext) -> str | None:
    active = latest_pending_action(
        FlorenceChannelLog(context.store),
        channel_id=context.channel_id,
    ...
    if active is None:
        return None
    return str(active.action.target_id or "").strip() or None
...
def _build_candidate_review_context(
    ...
    *,
    ...
    resolution: str,
    source_visibility: HouseholdSourceVisibility | None,
) -> dict[str, object]:
    active = latest_pending_action(
        FlorenceChannelLog(context.store),
        channel_id=context.channel_id,
    ...
    latest_assistant = active.message if active is not None else None
    latest_metadata = active.metadata if active is not None else {}
    action = active.action if active is not None else None
    prompt_created_at = None
    if latest_assistant is not None and latest_assistant.created_at is not None:
        prompt_created_at = datetime.fromtimestamp(float(latest_assistant.created_at), tz=timezone.utc).isoformat()
    review_context: dict[str, object] = {
        ...
        "prompt_message_id": latest_assistant.id if latest_assistant is not None else None,
        "prompt_created_at": prompt_created_at,
        "prompt_protocol_kind": latest_metadata.get("protocol_kind"),
        "prompt_preview": latest_assistant.body if latest_assistant is not None else None,
    }
        ...
        review_context.update(
            {
                ...
                "pending_action_target_kind": action.target_kind,
                "pending_action_target_id": action.target_id,
                "pending_action_target_ids": list(action.target_ids),
                "pending_action_expires_at": action.expires_at,
            }
        )
    return {key: value for key, value in review_context.items() if value is not None}


def _validate_pending_action_binding(
    context: FlorenceHouseholdToolContext,
    *,
    expected_action_type: str,
    target_id: str | None,
    requested_pending_action_id: str | None,
    ...
    latest_metadata = active.metadata
    if PENDING_ACTION_ID_KEY not in latest_metadata:
        # Legacy rows before explicit pending-action ids remain valid because
        # they still gate by currently surfaced target id below.
        return None
    if pending_action_is_expired(action):
        return {
            "error": "The active pending action has expired. Ask the parent to confirm the action again.",
            "active_pending_action_id": action.id,
            "active_pending_action_type": action.action_type,
            "active_pending_action_target_ids": list(action.target_ids),
        }
    if action.action_type != expected_action_type:
        return {
            "error": "The active pending action is for a different action type.",
            "active_pending_action_id": action.id,
            "active_pending_action_type": action.action_type,
            "expected_pending_action_type": expected_action_type,
        }
    if target_id and action.target_ids and target_id not in action.target_ids:
        return {
            "error": "The requested target is not part of the active pending action.",
            "active_pending_action_id": action.id,
            "active_pending_action_target_ids": list(action.target_ids),
            "requested_target_id": target_id,
        }
    if not requested_pending_action_id:
        return {
            "error": "Missing required parameter: pending_action_id for the currently surfaced pending action.",
            "active_pending_action_id": action.id,
            "active_pending_action_type": action.action_type,
            "active_pending_action_target_ids": list(action.target_ids),
        }
    if requested_pending_action_id != action.id:
        return {
            "error": "pending_action_id does not match the currently surfaced pending action.",
            "active_pending_action_id": action.id,
            "requested_pending_action_id": requested_pending_action_id,
            "active_pending_action_type": action.action_type,
            "active_pending_action_target_ids": list(action.target_ids),
        }
    return None
...
def _handle_apply_nudge_action(args: dict, *, task_id: str | None = None, **_: Any) -> str:
    ...
    binding_error = _validate_pending_action_binding(
        context,
        expected_action_type="household_nudge",
        target_id=nudge_id,
        requested_pending_action_id=_normalize_optional_text(args.get("pending_action_id")),
    )
    if binding_error is not None:
...
def _active_review_candidate_id(
    ...
    *,
    ...
    explicit_candidate_ids = active_pending_target_ids(active)
...
def _handle_search_google_inbox(args: dict, *, task_id: str | None = None, **_: Any) -> str:
    ...
    mirror_freshness = build_google_mirror_freshness_summary(
        connection_freshness=connection_freshness,
        source_kind="gmail",
    )
